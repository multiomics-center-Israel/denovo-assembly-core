#!/usr/bin/env bash
# A15_fcs_gx — FCS-GX (clean). Frozen assembly record/verifier (logic in legacy pipeline.py).
export PIPE_DOMAIN="assembly"; STEP_ID="A15_fcs_gx"
source "${PROJECT_ROOT:-/mnt/data/Projects/Elad_Chiel/wasp_genome_assembly}/annotation/steps/_lib.sh"
OUT="${PROJECT_ROOT}/NCBI_FCS_GX_dataset_1.Action_report.txt"
step_check() { [[ -e "$OUT" ]]; }
step_run() {
