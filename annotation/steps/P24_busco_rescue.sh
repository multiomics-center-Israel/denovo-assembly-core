#!/usr/bin/env bash
# P24_busco_rescue — miniprot BUSCO rescue. [DONE] verify-and-record wrapper (logic in existing drivers).
export PIPE_DOMAIN="annotation"; STEP_ID="P24_busco_rescue"
source "$(dirname "$0")/_lib.sh"
OUT="${PROJECT_ROOT}/annotation/busco_rescue"
step_check() { [[ -e "$OUT" ]]; }
step_run() {
  if step_check; then
